...
export function getComponentByType (args, cliConfigFile) {
  ...
  ${chalk.bold('nrx.conf.json')} config file under the ${chalk.bold(
            'component'
          )} object.
              `
        )
...
function getCustomTemplate (componentName, templatePath) {
  ...
  try {
    ...
    const filename = path
      ...
      .replace(/template[_-]?name/i, componentName)
...
Please make sure you're pointing to the right custom template path in your nrx.conf.json config file.
        `
      )
    )
...
function componentDirectoryNameGenerator ({
  cmd,
  componentName,
  cliConfigFile,
  filename,
    ...
    if (customDirectoryConfigs.length > 0) {
      ...
      for (const convertor in convertors) {
        ...
        if (re.exec(customDirectory) !== null) {
          componentDirectory = customDirectory.replace(
            convertor,
...
function componentTemplateGenerator ({
  ...
  return {
    componentPath: componentDirectoryNameGenerator({
      cmd,
      componentName,
      cliConfigFile,
      filename,
